...
const ResumeCard: React.FC<ResumeCardProps> = ({ dateRange, jobTitle, company, location, children, className = '' }) => {
  return (
    <div className={`shadow-slate-300 shadow-lg border border-slate-200 bg-white p-12 rounded-xl w-full flex justify-between items-center gap-8 ${className}`}>
      <div className='bg-slate-50 p-6 rounded-xl mb-4 w-52 h-40'>
      ...
      <div className='flex-1 p-2 text-slate-600 text-md'>
      {children}
      </div>
    </div>
  );
